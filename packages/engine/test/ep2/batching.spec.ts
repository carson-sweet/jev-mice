...
describe('US-E02-02 Requests are batched by proximity', () => {
  ...
  it('Each mouse is addressed by its own key', () => {
    ...
    const [req] = composeRequests(w, ready)
  ...
  it('Cats are never mixed with mice', () => {
    ...
    for (const req of composeRequests(w, ready)) {
  ...
  it('Grouping is deterministic', () => {
    ...
    const a = composeRequests(w, ready).map((r) => r.agents)
    const b = composeRequests(w, ready).map((r) => r.agents)
    expect(b).toEqual(a)
  })
  ...
  it('Grouping keeps mice near one another', () => {
    ...
    for (const req of composeRequests(w, ready)) {
      ...
      expect(Math.max(spanX, spanY),
        'a batch spanned more of the map than proximity grouping should allow').toBeLessThan(40)
    }
  })
})
