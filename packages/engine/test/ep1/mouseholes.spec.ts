...
import {
  availableDrives, NUTRITION_BANDS, PUP_NUTRITION, type Drive,
} from '../../src/index.js'
...
describe('US-E01-06 Mouseholes shelter, breed and raise pups', () => {
  ...
  it('A litter is born into a hole and emerges when hunger takes the pups out', { timeout: 30_000 }, async () => {
    // The wait is not a constant of the engine: pups start full and leave when
    // they drop below the fed band, so the window is set by the decay rate.
    // This asserted 25 to 40 ticks, which was that arithmetic at the old
    // default decay of 0.5, and broke when the default became 0.3. Derived
    // here instead, so the test states the rule rather than one of its values.
    const decay = 0.3
    const expected = (PUP_NUTRITION - NUTRITION_BANDS.fed) / decay
    const evs = await runTicks(engine(medium({
      cats: 0, mouseholes: 30, foodPiles: 60, nutritionDecayPerTick: decay,
    })), 2000)
    ...
    expect(freed!.tick - brood!.tick).toBeGreaterThanOrEqual(expected - 5)
    expect(freed!.tick - brood!.tick).toBeLessThanOrEqual(expected + 10)
  })
