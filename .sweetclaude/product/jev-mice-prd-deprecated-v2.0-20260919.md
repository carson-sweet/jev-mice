...
audience: hybrid
nda: false
changes: major. Added mouseholes as an environment entity (shelter, hiding, breeding site, pup safety); resolved seven of eight open questions (intent label, cat prowl, hide, fear effect, rumor hops, batching regions, pup safety); functional requirements renumbered
previous_file: jev-mice-prd-deprecated-v1.0-20260919.md
---
...
[9] **Mouse decision request.** One request per decision, containing the questions below. Questions marked speculative are always asked when their precondition holds and are read only when the drive answer makes them relevant. Mice inside holes make no requests.
...
[11] **Batching rule.** Up to 8 mice from the same 16-by-16 tile share one request. Each mouse's state sits under its own key, and every question's instruction names that key. Cats are never batched with mice.

[12] **Baseline substitution.** When Jev is off or a decision falls back, the same questions are answered by fixed rules: drive weights from nutrition band, danger presence, and shelter availability; fear from nearest cat distance; approach suspect food as no when a death memory exists; mate choice as nearest candidate; nest site as safe when no cat has been perceived within 100 ticks. The event records which source answered.
...
[22] **Constraints.** TypeSafe's published limits: 1,200 requests per minute, 250,000 tokens per second, 64,000 tokens per request with 32,000 for state plus the longest question, text-only input. Published price $0.042 per million input tokens, described as an early-access snapshot. Jev is documented as weak at arithmetic, counting, numeric comparison, and large states full of irrelevant detail, and its Score probabilities are not to be interpolated into magnitudes. The TypeSafe JavaScript SDK is MIT licensed. Storage is browser-local. One user, one browser, one run at a time.

## 10. Open Questions

[23] **Open.** The license for the showcase repository. Carson has chosen to decide later. The SDK's MIT license places no constraint on the choice. An unlicensed public repository is all-rights-reserved by default, so this must be settled before publication.
...
[25] Full user stories with acceptance criteria for the six epics. User flows and wireframes for the configuration screen, live view, inspector, and comparison view. System architecture, module boundaries, and the proxy's request and response contract. The event schema as a formal type definition. The exact Jev instruction and criteria wording for each question, with the tested bucket boundaries and the shelter descriptions. The baseline rule weights. A test strategy including how SM-06 and SM-07 are computed from a record. An accessibility audit beyond the baseline. Licensing. Toggles deferred from this version: hole exit for reasons other than hunger, rumor chains with decay, personality inheritance and the lineage view it enables. A mobile layout and a server-side store if the showcase outgrows browser storage.
