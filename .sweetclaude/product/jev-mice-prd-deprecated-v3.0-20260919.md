...
audience: hybrid
nda: false
changes: major. Rewritten for a hosted multi-tenant web application with server-side simulation. Twelve requirements that described a single-user browser app are revised or withdrawn; ten new requirement groups cover accounts, run lifecycle, server execution and resume, record storage, library and export, sharing, quotas, public mode, account data, and operations. Success metrics and non-functional requirements rewritten. Simulation mechanics carried forward unchanged.
