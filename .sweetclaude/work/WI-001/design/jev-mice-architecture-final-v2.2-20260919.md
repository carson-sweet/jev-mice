...
audience: hybrid
nda: false
changes: minor. Applies the solution validation remediation: shared request budget and spatial-sort batching, presigned object URLs replacing container credentials, viewer state from the running simulation, per-chunk summary segments, and the fear, control, quota, session and privacy corrections. ADR-021 to ADR-024 added. Approved as final by Carson Sweet on 2026-09-19.
previous_file: jev-mice-architecture-superseded-v2.1-20260919.md
---
...
**Nothing holds a run in memory.** Events leave the container in gzipped chunks, each with its own summary segment. Charts concatenate segments. Viewers receive frames. Replay pages through chunks. The largest thing any process holds is one chunk and its segment, and that is true of the analysis views as well as the simulation.
...
**Cost is bounded from three directions**, per subject, per address and per deployment, every applicable one checked once per chunk and the first refusal deciding. **Request rate is bounded once**, by a single deployment budget every simulation draws from, so throughput falls as concurrency rises rather than the published limit being exceeded. Exhaustion degrades to code-only rules with a banner; it never breaks a run.
...
| Run records | Gzipped chunks in R2 (250 ticks or 8 MB, whichever first), one summary segment per chunk, chunk index in Postgres | ADR-014, ADR-020, ADR-024 |
...
| Sign-in | Google via @hono/oauth-providers, KV sessions with a per-account index, AUTH_REQUIRED flag | ADR-005 |
| Object access | The coordinator mints presigned single-object URLs; containers hold no storage credential | ADR-022 |
| Viewer state | Container world-state endpoint plus the coordinator's last broadcast frame | ADR-023 |
| Public mode | Server-side execution, 24-hour anonymous retention, no sharing | ADR-015 |
| Quotas and rate | QuotaCounter per subject; a GlobalLimits singleton holding budget, capacity, queue and the shared request-rate budget; allowance and rate leased together per chunk | ADR-007, ADR-016, ADR-021 |
| Jev client | @typesafe-ai/sdk 0.6 in the container, key in container environment | ADR-016 |
| Relational store | Neon Postgres 17 via serverless driver, Drizzle ORM | ADR-004 as amended by ADR-014 |
| Browser | Vite, React 19, Tailwind 4, uPlot, canvas; viewer only | ADR-017 |
...
KV: sessions      Durable Objects: QuotaCounter per subject, GlobalLimits
```
...
**Chunk boundary.** When the chunk reaches 250 ticks or 8 MB uncompressed, whichever comes first, the container gzips it, its summary segment and the engine snapshot, and uploads each through a presigned URL the coordinator issued with the previous acknowledgement. It then posts sizes, counts and its usage report. The coordinator writes the chunk index row, updates the run row, records usage, reconciles every applicable budget, and answers with the next token allowance, the next share of the request budget, the current control state, and presigned URLs for the following boundary. On refusal the container switches to code-only decisions for the next chunk and emits a fallback event.
...
**Watch a run.** The browser opens a WebSocket. The Worker authorizes it and hands the socket to the Run object, which sends the current world state and then frames. That state is the most recent frame the object broadcast when it is under a second old, and a fetch from the container's world-state endpoint otherwise; it is never read from storage, which can be minutes stale. Pause, resume, step, speed and stop are messages to the object, which pushes them to the container at once rather than waiting for a boundary. A closed tab changes nothing, and reopening is the same path as connecting.
...
**HARD REQUIREMENT. No analytics or tracking.** No third-party script, pixel or fingerprinting in any page served to a person. Errors seen in the browser are posted to a first-party endpoint on this service and forwarded onward with the address removed; no reporting library talks to anyone else from the page. Telemetry describes simulated animals and the subject's own decision usage.

**HARD REQUIREMENT. Secrets stay server-side.** The decision key, the sign-in secret, the database URL, the session signing key and the object-store credential exist only as Worker secrets. A container receives the decision key, its own run's report token, and presigned single-object URLs; it holds no storage credential and cannot name a storage key. The report token is reissued on every container start, so a process the system has replaced cannot report into its replacement's run. The browser bundle and every API response contain no key material, checked in the build and in tests.
...
**HARD REQUIREMENT. Container reports are authenticated** with a token issued at each start and invalidated when a replacement starts, and a container may write only to the single objects the coordinator has presigned for it.
...
**apps/worker.** Hono app, session and auth-flag middleware, routes, the Run Durable Object, QuotaCounter and GlobalLimits Durable Objects, the Container binding, Drizzle schema and migrations, R2 access. Imports types from the engine and nothing else from it.
...
**Flags and limits** as Worker variables passed through to containers where relevant: AUTH_REQUIRED, JEV_ENABLED, OWNER_EMAILS, ANON_DAILY_TOKENS, USER_DAILY_TOKENS, IP_DAILY_TOKENS, GLOBAL_DAILY_COST_MICROS, JEV_PRICE_PER_MTOK_MICROS, MAX_ACTIVE_RUNS_PER_SUBJECT (1), MAX_ACTIVE_RUNS_GLOBAL (20), REQUEST_BUDGET_RPM (1000, below the published 1200), CHUNK_TICKS (250), CHUNK_MAX_BYTES (8 MB), FRAME_INTERVAL_MS (50), RUN_RETENTION_DAYS (30, owners exempt, badge in the last 7), ANON_RETENTION_HOURS (24), SWEEP_INTERVAL_HOURS (6), CONTAINER_INSTANCE_TYPE per preset (basic).

**Capacity.** Each active run is one container. Per-user and global active-run limits gate starts; a run beyond the global limit waits in status queued and the viewer shows its position. Every preset runs on the basic type (1/4 vCPU, 1 GiB); the per-preset setting exists so Large can move up after measurement. Throughput, unlike capacity, is shared rather than gated: the running simulations divide one request budget, so a quiet deployment is fast and a busy one is slow. Measured on Medium, two ticks per second holds to five concurrent runs and falls to about half a tick per second at twenty.
...
**Failure posture.** TypeSafe unavailable or budget exhausted: baseline for the affected batch or chunk, banner, fallback events. Container dies mid-run: the Run object issues a fresh report token and a presigned read for the last snapshot, starts a replacement, and the run continues from the first tick after the last written chunk. Chunks already written stand, and a late report from the old process is rejected. Neon unavailable: sign-in and library fail with a clear message; running containers keep writing chunks to R2 and the Durable Object replays the row writes when Neon returns. R2 unavailable: the container retries the chunk post with backoff and pauses the loop after a bounded buffer.
...
- ADR-001: Pure TypeScript engine with a seeded generator and no platform dependencies (amended) (`adr/ADR-001-pure-typescript-engine-20260919.md`)
...
- ADR-011: Share links as unguessable revocable tokens; account deletion cascades (amended) (`adr/ADR-011-share-links-and-deletion-cascade-20260919.md`)
...
- ADR-013: Engine executes server-side in a Cloudflare Container per run, orchestrated by a Run Durable Object (partially superseded) (`adr/ADR-013-server-side-engine-container-per-run-20260919.md`)
- ADR-014: Run records as gzipped tick-range chunks in R2 with a compact summary series; nothing buffered in memory (partially superseded) (`adr/ADR-014-chunked-run-records-summary-series-20260919.md`)
- ADR-015: Public mode runs server-side with 24-hour anonymous retention and no sharing (amended) (`adr/ADR-015-public-mode-server-side-24h-retention-20260919.md`)
...
- ADR-019: Engine snapshot at every chunk boundary and automatic resume after container failure (partially superseded) (`adr/ADR-019-engine-snapshot-and-resume-20260919.md`)
...
- ADR-021: Spatial-sort batching and a shared deployment request budget (`adr/ADR-021-capacity-shared-request-budget-20260919.md`)
- ADR-022: The coordinator mints presigned object URLs; containers hold no storage credential (`adr/ADR-022-presigned-object-urls-20260919.md`)
- ADR-023: The viewer's opening state comes from the container through a cached frame (`adr/ADR-023-viewer-state-from-container-20260919.md`)
- ADR-024: The summary series is stored as per-chunk segments (`adr/ADR-024-chunked-summary-segments-20260919.md`)

## 11. Open questions

None. The six left open in version 2.1 were settled in ADR-020, and the seven blocking findings of the solution validation are settled in ADR-021 to ADR-024 and in the revisions applied here.
