...
[1] For everyone when authentication is required. Entry point: any app URL without a session.
...
[2] Success: run library with the user's avatar and name in the header, remaining daily Jev budget shown. Errors: the two above, both recoverable by retrying.

## F-02 Arrive in public mode

[3] For anonymous visitors when authentication is off. Entry point: any app URL.
...
[4] Success: configure screen ready. There is no library, no account screen, and no share action anywhere in the interface in this mode. Errors: none specific; quota states are covered in F-12.

## F-03 Configure and start a run

[5] For the watcher and the researcher above all; Carson uses it to hit limits on purpose. Entry point: "New run" in the header or the library's empty state. Requirements FR-053 to FR-056, FR-003, FR-043.
...
[6] Success: live view showing tick 0 and the first frames. Errors: validation messages inline; the three start-time checks each offer a way forward rather than a dead end; if the container fails to start within 30 seconds the run shows "Could not start. Try again." and the row is marked failed.

## F-04 Watch a live run

[7] For everyone. Entry point: a run starting (F-03), the library's Open on a running run, or a reconnect. Requirements FR-064, FR-066 to FR-068.
...
[8] Success: a completed run with the full record. Errors: every banner above is informational; only the double resume failure ends a run early.

## F-05 Inspect an animal

[9] For the evaluator and Carson. Entry point: clicking any animal in the live view or in replay. Requirement FR-065.
...
[10] Success: the evaluator can read what Jev saw and what it said for one animal at one moment. Errors: in replay, if the chunk holding the decision has not loaded yet the section shows a loading state, never stale data from another tick.

## F-06 Leave and come back

[11] For the watcher and the researcher running long runs. Entry point: closing the tab or navigating away during a run.
...
[12] Success: the run is exactly where it would have been had the tab stayed open. Errors: if the run completed while away, Open lands on the completed state with Replay available.

## F-07 Run library

[13] For signed-in users. Entry point: the header's Runs link; the landing page after sign-in.
...
[14] Success: every run the user owns is one click from replay, comparison, or sharing. Errors: a run that expired between page load and click returns "This run has expired" and disappears from the list on refresh.

## F-08 Replay a completed run

[15] For everyone. Entry point: Open on a completed, failed, or cancelled run; the Replay control on a finished live view; a share link. Requirements FR-062, FR-065.
...
[16] Success: any tick of any run the user may see, with the same inspector fidelity as live. Errors: chunk load failures are local to the range and retryable.

## F-09 Compare two runs

[17] For the watcher and the researcher. Entry point: Compare on two library rows, or "Compare with" on a run detail. Requirement FR-063.
...
[18] Success: the 70 percent bold run beside the 30 percent bold run with the differing field highlighted and the curves overlaid. Errors: none beyond the warnings.

## F-10 Share a run

[19] For signed-in owners and anyone they send the link to. Entry point: Share on a library row or run detail.
...
[20] Success: a recipient with no account watches or replays the run. Errors: one not-found page.

## F-11 Export a run

[21] For the researcher above all, and for anyone who wants to keep a run past 30 days. Entry point: Export on a library row or run detail; Export on the configure screen for configuration only.
...
[22] Success: a file the researcher can archive or hand to someone else. Errors: interruptions are restartable.

## F-12 Quota and budget states

[23] For everyone, most visibly in public mode. Entry point: the header's budget indicator, and any point where a limit is reached.
...
[24] Success: the user always knows why a run is on baseline rules and when Jev returns. Errors: none; these are states, not failures.

## F-13 Account

[25] For signed-in users. Entry point: avatar in the header.
...
[26] Success: a clean account and a clean exit. Errors: the all-or-nothing rule above.

## F-14 Queue

[27] For everyone when the deployment is busy. Entry point: Start when 20 containers are active.
...
[28] Success: a fair wait with a visible position. Errors: none.

## F-15 Baseline twin

[29] For the evaluator and Carson: the showcase's central comparison. Entry point: "Run baseline twin" on a completed Jev run's detail.
...
[30] Success: two curves on one chart that differ only because one colony had judgment. Errors: as in F-03 and F-09.

## F-16 Public-mode run lifetime

[31] For anonymous visitors. Entry point: any anonymous run.
...
[32] Success: a complete, short-lived experience with no account. Errors: none beyond expiry.

## Open questions

[33] Whether the compare tray should persist across sessions or reset on reload. Reset is the proposal.

[34] Whether a share viewer should be able to open the inspector. Yes is the proposal, since the inspector is the point of the showcase.

[35] Whether Rename should exist in the first version or names stay derived from preset and date. Include is the proposal; it is one field.

[36] Whether the configure screen's cost estimate should also show expected wall-clock duration at the current Jev latency. Yes is the proposal, labeled as an estimate.
